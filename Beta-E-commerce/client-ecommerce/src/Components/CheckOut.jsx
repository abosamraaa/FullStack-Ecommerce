...
function CheckOut() {
  ...
  const productsIds = sessionStorage.getItem("ids");
  const parsedIds = JSON.parse(productsIds);
  console.log(parsedIds)
  const orderProductQuantityList = parsedIds.map((id) => ({
    productId: id,
    quantity: 1, // Default quantity (can be adjusted based on your requirements)
  }));
  const placeOrder = async () => {
    try {
      const response = await fetch("http://localhost:8090/placeOrder/false", {
        method: "POST",
        headers: {
